    ...
    public class pObjectManager
    {
        ...
        internal void updateModel()
        {
            if (!isUpdating)
            {
                isUpdating = true;
                isAgainUpdate = false;
                if (earth.config.isDynShow && earth.camera.operateMode == EOperateMode.地图模式)
                {
                    earth.camera.cameraFrustum = new BoundingFrustum(earth.camera.view * Matrix.CreatePerspectiveFieldOfView(MathHelper.PiOver4 * 0.9f, (float)earth.global.ScreenWidth / earth.global.ScreenHeight, earth.camera.Near, earth.camera.Far));
            ...
            foreach (PowerBasicObject item in Models)
            {
                if (item is pPowerLine)  //线路模型
                {
                    ...
                    STRUCT_Line strupara = new STRUCT_Line()
                    {
                        ...
                        color = Helpler.ColorToUInt(obj.color),
                    ...
                    if (obj.showType == pData.EShowType.几何体)
                    {
                        foreach (Data zd in obj.datas)
                        {
                            ...
                            STRUCT_PolyCol strupara = new STRUCT_PolyCol
                            {
                                ...
                                rootid = rootid,
                                parentid = parentid,
                                color = Helpler.ColorToUInt(zd.color),
                        ...
                        if (obj.isShowLabel) //显示标签
                        {
                            ...
                            STRUCT_Symbol strupara = new STRUCT_Symbol
                            {
                                ...
                                color = Helpler.ColorToUInt(obj.dataLabelColor),
                                axis = obj.axis,
                                angle = obj.angle,
                            };
                            IntPtr ipPara = Marshal.AllocCoTaskMem(Marshal.SizeOf(strupara));
                ...
                else if (item is pCustomObject)  //自定义模型对象
                {
                    ...
                    if (obj.VecVertices.Count() > 0 && obj.VecIndexes.Count() > 0)
                    {
                        ...
                        //normals数据
                        IntPtr ipNormal = Marshal.AllocCoTaskMem(Marshal.SizeOf(obj.VecNormals[0]) * count);  //传递点序列结构数组指针
                        for (int i = 0; i < count; i++)
                        {
        ...
        ///<summary>添加XModel模型</summary>
        public bool AddXModel(string key, string file)
        {
            if (zXModels.Keys.Contains(key))
                return false;
            else
                zXModels.Add(key, new pXModel() { id = key, filepath = file });
            return true;
        }
