...
namespace WpfEarthLibrary
{
    ...
    public partial class Earth : UserControl
    {
        ...
        {
            if (isEnable)
                Application.Current.MainWindow.MouseDoubleClick += new MouseButtonEventHandler(MainWindow_MouseDoubleClick);
            else
                Application.Current.MainWindow.MouseDoubleClick -= new MouseButtonEventHandler(MainWindow_MouseDoubleClick);
        }
        ///<summary>借用主窗体实现双击</summary>
        ...
        void CompositionTarget_Rendering(object sender, EventArgs e)
        {
            ...
            if (d3dimg.IsFrontBufferAvailable && _lastRender != args.RenderingTime)
            {
                ...
                if (pSurface != IntPtr.Zero)
                {
                    ...
                    if (earthManager.mapType != EMapType.无)
        ...
        private void grdMain_MouseWheel(object sender, MouseWheelEventArgs e)
        {
            ...
            if (e.Delta > 0)
            {
                if (distanceground < camera.MaxGroundDistance)
                {
                ...
                if (distanceground > camera.MinGroundDistance)
                {
                    camera.cameraPosition += camera.cameraDirection * distanceground * 0.1f;
                    isupdate = true;
                }
            if (isupdate)
            {
                ...
                if (config.isShowDebugInfo && camera.operateMode == EOperateMode.地图模式) showdebuginfo();
        ...
        void showdebuginfo()
        {
            ...
            string info1 = string.Format(s1, camera.curCamearaViewRange.nearLongitudeStart, camera.curCamearaViewRange.nearLongitudeEnd,
