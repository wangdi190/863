...
namespace WpfEarthLibrary
{
    ...
    ///<summary>地图模式禁止Y轴旋转确保文字向上适用于地理图；轨迹球模式可自由旋转适用于呈现模型物体</summary>
    public enum EOperateMode {地图模式,轨迹球模式,自由视角}

    ///<summary>对象可见性检查模式</summary>
    public enum ECheckMode {视锥体检查,经纬检查}

    internal enum EModelType { 地图, 相机, 光源, 折线, 图元, 几何体, 区域, 等值图, 文字, 潮流 ,自定义模型};
    internal enum EPropertyType { 类型, 颜色, 大小, 长度, 宽度, 高度, 偏移, 方向, 材质, 动画, 内容, 可见性, 参数, 地址, 路径, 路径2, 进度, 位置 ,角度};
