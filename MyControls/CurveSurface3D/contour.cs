...
namespace MyControlLibrary.Controls3D.CurveSurface3D
{
    ...
    public class Contour
    {
        ...
        /// <summary>
        /// 是否边缘结点强制设置为0，以生成无边框的图形
        /// </summary>
        public bool isEdgeSetValue = false;
        ...
        //=========下为方便调试用参数
        bool isShowGridData = false; //是否绘制网格节点数据
        bool isShowData = false; //是否绘制原始数据
        bool isDebug = false;
        ...
        {
            if (isEdgeSetValue && !isPreHandled)  //包络插值
            {
                // ========== 求出包络图形，简单凸包，提高性能 ==========
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    dots.Add(new ValueDot() { value = EdgeValue, location = pts[i] });
                    int devcount = (int)(len / div);
                    if (devcount > 0)
                    {
                        double step = len / devcount;
                        for (int j = 0; j < devcount; j++)
                        {
                            dots.Add(new ValueDot() { value = EdgeValue, location = pts[i] + j * step * vec });
                ...
                for (int i = 0; i < gridXCount + 1; i = i + 2)
                {
                    for (int j = 0; j < gridYCount + 1; j = j + 2)
                    {
                        if (i == 0 || i == gridXCount || j == 0 || j == gridYCount)
                        {
                            ...
                            dots.Add(new ValueDot() { value = EdgeValue, location = location });
                        }
                    }
                }
        ...
        private void Draw()
        {
            ...
            if (tmpLine != null && tmpLine.vltype == ValueLine.EVLType.计算并且绘制)
            {
            ...
            //foreach (Point pnt in pts)
            //    pcs.Add(pnt);
            //canv.Children.Add(new Polygon() { Points = pcs, Stroke = Brushes.Purple, StrokeThickness = 10 });
            //foreach (Point pnt in tmpdots)
            //{
            //    canv.Children.Add(new Ellipse() { Width = 10, Height = 10, Fill = new SolidColorBrush(Colors.White), Margin = new Thickness(pnt.X - 5, pnt.Y - 5, 0, 0) });
            //}
        }
